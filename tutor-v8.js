(() => {
  ...
  function localNotice(){ return configured() ? '' : '<p class="ai-fallback-note">🟢 وضع مجاني موسّع — يعمل بدون اشتراك API، ويحفظ التقدم والأخطاء على الجهاز.</p>'; }
  ...
  function topicHome(i){
    ...
    setLesson(`<article class="lesson-card"><div class="lesson-kicker">${escapeHtml(t.name)}</div><h4>${escapeHtml(s.title)} • جاهزة نبدأ؟</h4><p>${escapeHtml(t.summary)}</p><div class="study-stats"><span>🎯 الإتقان الحالي <b>${mastery}%</b></span><span>🧪 الاختبار <b>12 سؤالًا متجددًا</b></span></div><p class="memory-tip">ابدئي بـ «اشرح لي»، ثم مثال، وبعدها الاختبار. عند الخطأ سيحفظ DURRA نقطة الضعف للمراجعة القادمة.</p><button id="topicBack" class="mini-cta soft-local" type="button">رجوع إلى قائمة ${escapeHtml(s.kind)}</button></article>`);
    document.getElementById('topicBack')?.addEventListener('click',subjectHome);
  }

  function fallbackExplainHtml(t, level='normal'){
    const s=currentSubject();
    const simple = level==='simpler';
    const deep = level==='deeper';
    const title = simple ? '🌱 شرح أبسط جدًا' : deep ? '🔎 شرح أعمق ومفصل' : '📖 شرح موسّع خطوة بخطوة';
    const extra = deep
      ? `<div class="explain-steps"><h5>5) تحليل أعمق</h5><ol><li>حددي ما الذي يصفه القانون أو القاعدة، وما الذي يتغير وما الذي يبقى ثابتًا.</li><li>قارني بين الفكرة الأساسية والمثال حتى تعرفي متى تستخدمينها في السؤال.</li><li>انتبهي للوحدات والإشارات والكلمات المفتاحية لأنها غالبًا تحدد طريقة الحل.</li><li>بعد الحل، اسألي نفسك: هل النتيجة منطقية؟ وهل توافق الفكرة الأساسية؟</li></ol></div>`
      : `<div class="explain-steps"><h5>5) طريقة الحفظ والفهم</h5><ol><li>افهمي المعنى أولًا، لا تحفظي الجملة فقط.</li><li>قسّمي الفكرة إلى أجزاء صغيرة.</li><li>اربطي كل جزء بمثال.</li><li>اختبري نفسك من دون النظر للشرح.</li></ol></div>`;
    const simpleText = simple ? `<p class="memory-tip">✨ باختصار شديد: ${escapeHtml(t.summary)}<br><b>احفظي المفتاح:</b> ${escapeHtml(t.key)}</p>` : '';
    return `<article class="lesson-card">${localNotice()}<div class="lesson-kicker">${escapeHtml(t.name)}</div><h4>${title}</h4>${simpleText}<section class="ai-section"><h5>1) ما الفكرة؟</h5><p>${escapeHtml(t.summary)}</p></section><section class="ai-section"><h5>2) ما الذي يجب أن أتذكره؟</h5><p>ركزي على العلاقة أو القاعدة الأساسية التالية، وحاولي معرفة معنى كل جزء فيها بدل حفظ شكلها فقط.</p></section><div class="law-box"><span>القانون / القاعدة الأساسية</span><strong>${escapeHtml(t.key)}</strong></div><section class="ai-section"><h5>3) كيف أستخدمها في السؤال؟</h5><p>اقرئي المطلوب أولًا، ثم استخرجي المعطيات أو الكلمات المفتاحية، وبعدها اختاري القانون أو القاعدة المناسبة وطبقيها بالترتيب.</p></section><div class="answer-box"><b>4) مثال موضح:</b><br>${escapeHtml(t.example)}</div>${extra}<div class="try-box"><h5>🎯 اختبري فهمك بسرعة</h5><p>اشرحي الفكرة بجملة من عندك، ثم غطي القانون وحاولي كتابته من الذاكرة، وبعدها انتقلي إلى «مثال» أو «اختبار سريع».</p></div><div class="result-actions"><button id="simplerExplain" type="button">🌱 شرح أبسط</button><button id="deeperExplain" type="button">🔎 شرح أعمق</button></div></article>`;
  }
  ...
  async function showExplain(level='normal'){
    ...
    setLesson(fallbackExplainHtml(t,level)); bindExplainDepthButtons();
  }
  ...
  async function showExample(){
    ...
    setLesson(`<article class="lesson-card">${localNotice()}<div class="lesson-kicker">${escapeHtml(t.name)}</div><h4>🧩 3 تطبيقات للتثبيت</h4><section class="worked-example"><h5>مثال 1 — افهمي الطريقة</h5><p>${escapeHtml(t.example)}</p></section><section class="worked-example"><h5>مثال 2 — طبقي نفس الفكرة</h5><p>بدّلي رقمًا أو كلمة أو معطى واحدًا في المثال السابق، ثم أعيدي الخطوات نفسها من البداية.</p><ol class="solve-steps"><li>حددي المطلوب.</li><li>استخرجي المعطيات أو الكلمات المفتاحية.</li><li>اختاري ${escapeHtml(t.key)}.</li><li>طبقي ثم راجعي النتيجة.</li></ol></section><section class="worked-example"><h5>مثال 3 — سؤال تحدي</h5><p>حاولي تفسير لماذا نستخدم هذه القاعدة أو العلاقة هنا، وما الذي سيتغير إذا تغير أحد المعطيات أو الكلمات الأساسية.</p></section><div class="try-box"><b>🎯 الآن دورك:</b><p>اكتبي الحل أو الفكرة من ذاكرتك قبل فتح الاختبار.</p></div><button id="goQuiz" class="mini-cta" type="button">اختبار متجدد الآن ⚡</button></article>`);
    document.getElementById('goQuiz')?.addEventListener('click',startTopicQuiz);
  }
  ...
  function fallbackQuiz(t){
    ...
    const generic=[
      q('أي عبارة تلخص الفكرة الأساسية لهذا الجزء؟',[t.summary,'لا توجد فكرة محددة في هذا الجزء.','الموضوع منفصل عن المادة تمامًا.','كل الخيارات السابقة صحيحة.'],0,'هذه العبارة تلخص الفكرة الأساسية للدرس.'),
      q('ما العلاقة أو القاعدة التي يجب تذكرها أولًا؟',[t.key,'لا توجد قاعدة أو فكرة أساسية.','يكفي حفظ عنوان الدرس فقط.','نستخدم أي قانون عشوائي.'],0,'هذه هي العلاقة أو القاعدة الأساسية في هذا الجزء.'),
      q('أي مثال يرتبط مباشرة بما تعلمناه؟',[t.example,'مثال لا علاقة له بموضوع الدرس.','تغيير اسم الفصل فقط.','حفظ العنوان دون تطبيق.'],0,'هذا المثال يطبق فكرة الدرس مباشرة.'),
      q('قبل البدء بالحل، ما الخطوة الأفضل؟',['تحديد المطلوب والمعطيات أو الكلمات المفتاحية','اختيار جواب عشوائي','تجاهل السؤال','حفظ الخيارات فقط'],0,'تحديد المطلوب والمعطيات يقود إلى اختيار الطريقة الصحيحة.'),
      q('بعد اختيار القانون أو القاعدة، ماذا نفعل؟',['نطبقه بالترتيب ثم نراجع النتيجة','نغيره بلا سبب','نتوقف عن الحل','نهمل الوحدات والمعنى'],0,'التطبيق المنظم ثم المراجعة يقللان الأخطاء.'),
      q('إذا أخطأتِ في هذا الموضوع، ما أفضل مراجعة؟',['معرفة سبب الخطأ ثم حل سؤال مشابه جديد','حفظ الجواب فقط','ترك الموضوع نهائيًا','تكرار الاختيار نفسه'],0,'فهم سبب الخطأ ثم التدريب بسؤال جديد أفضل طريقة للتثبيت.'),
      q('كيف تتأكدين أنك فهمتِ الفكرة فعلًا؟',['تشرحينها بكلماتك من دون النظر','تقرئين العنوان فقط','تحفظين لون الصفحة','تتجنبين التطبيق'],0,'الاسترجاع والشرح من الذاكرة دليل أقوى على الفهم.'),
      q('ما الذي يجب مراجعته عند نهاية الحل؟',['هل الجواب يوافق الفكرة والمعطيات','شكل الخط فقط','عدد كلمات السؤال','مكان السؤال في الصفحة'],0,'مراجعة منطق الجواب وعلاقته بالمعطيات خطوة مهمة.'),
      q('أي ترتيب دراسي أفضل لهذا الدرس؟',['فهم → مثال → اختبار → مراجعة الخطأ','اختبار عشوائي → ترك الدرس','حفظ العنوان فقط','قراءة سريعة بلا تطبيق'],0,'هذا التسلسل يربط الفهم بالتطبيق والمراجعة.'),
      q('عند رؤية سؤال جديد من نفس الموضوع، ماذا نبحث أولًا؟',['الكلمات أو المعطيات التي تربطه بالفكرة الأساسية','أطول خيار فقط','آخر رقم في السؤال','أي كلمة غير مهمة'],0,'الكلمات والمعطيات المفتاحية تساعد على التعرف إلى الفكرة المطلوبة.'),
      q('ما أفضل طريقة لتثبيت القانون أو القاعدة؟',['فهم معناها ثم استخدامها في أكثر من تطبيق','تكرار شكلها دون فهم','ترك الأمثلة','حفظها مرة واحدة فقط'],0,'الفهم مع التطبيق المتكرر يجعل الاسترجاع أقوى.'),
      q('إذا تغيرت معطيات المثال، ماذا نفعل؟',['نحافظ على الفكرة ونطبقها على المعطيات الجديدة','نستخدم جواب المثال نفسه دائمًا','نلغي القانون','نتجاهل التغير'],0,'الفكرة نفسها قد تبقى صالحة لكن التطبيق يتغير حسب المعطيات.')
    ];
    const pool=[...base,...generic];
    for(let i=pool.length-1;i>0;i--){ const j=Math.floor(Math.random()*(i+1)); [pool[i],pool[j]]=[pool[j],pool[i]]; }
    return pool.slice(0,12).map(shuffledQuestion);
  }
  ...
  async function startTopicQuiz(){
    ...
    if(configured()){
      loadingCard('أجهز 12 سؤالًا متنوعًا وأركز على أخطائك السابقة…');
      try{
        const data=await aiCall('quiz',{count:cfg().quizCount||12});
        const items=validateQuizQuestions(data.questions);
        if(items.length>=5){ state.quizItems=items.slice(0,12).map(shuffledQuestion); state.quizSource='ai'; renderTopicQuiz(); return; }
  ...
  async function showReview(){
    ...
    setLesson(`<article class="lesson-card">${localNotice()}<div class="lesson-kicker">${escapeHtml(t.name)}</div><h4>🔁 مراجعة مركزة</h4><div class="flashcard"><p>حاولي تذكر الفكرة أولًا 👀</p><button id="revealKey" type="button">أظهر الفكرة / القانون</button><strong id="hiddenLaw" class="hidden-law">${escapeHtml(t.key)}</strong></div>${wrongHtml}<button id="reviewQuiz" class="mini-cta" type="button">اختبار جديد 12 سؤالًا ⚡</button></article>`);
  ...
  async function askTutor(){
    ...
    let focus='';
    if(/قانون|معادلة|علاقة|formula/.test(lower)) focus=`<div class="law-box compact"><span>القانون / القاعدة</span><strong>${escapeHtml(t.key)}</strong></div><p>حاولي تحديد معنى كل جزء، ثم اربطيه بالمطلوب في السؤال.</p>`;
    else if(/مثال|تطبيق|حل/.test(lower)) focus=`<p><b>مثال من الدرس:</b> ${escapeHtml(t.example)}</p><ol><li>حددي المطلوب.</li><li>استخرجي المعطيات أو الكلمات المفتاحية.</li><li>طبقي القاعدة: ${escapeHtml(t.key)}</li><li>راجعي النتيجة.</li></ol>`;
    else if(/ليش|لماذا|فهم|اشرح|شنو|ما هو|ما هي/.test(lower)) focus=`<p><b>الفكرة ببساطة:</b> ${escapeHtml(t.summary)}</p><p>المفتاح الذي يربط أجزاء الموضوع هو: <b>${escapeHtml(t.key)}</b></p>`;
    else focus=`<p>${escapeHtml(t.summary)}</p><div class="law-box compact"><strong>${escapeHtml(t.key)}</strong></div><p>مثال يساعدك: ${escapeHtml(t.example)}</p>`;
    if(askAnswer) askAnswer.innerHTML=`<h5>💬 المدرّس المحلي المجاني</h5>${focus}<p class="memory-tip">🧠 أعيدي صياغة سؤالك بكلمات مثل «اشرح»، «القانون»، «مثال» أو «حل» حتى أختار لكِ نوع المساعدة الأنسب.</p>`;
  }
  ...
  window.DURRA_TUTOR={version:'8.0-free',openSubject,checkAiHealth};
